...
$fpdf_tpl_path = function_exists('base_path')
    ? base_path('vendor/iio/libmergepdf/tcpdi/fpdf_tpl.php')
    : __DIR__ . '/../../vendor/iio/libmergepdf/tcpdi/fpdf_tpl.php';
require_once $fpdf_tpl_path;

require_once __DIR__ . '/CustomTcpdiParser.php';


class CustomTCPDI extends \FPDF_TPL {
    /**
     * Actual filename
     * @var string
     */
    ...
    public function setSourceData($pdfdata) {
        ...
        if (!isset($this->parsers[$filename]))
            $this->parsers[$filename] = new CustomTcpdiParser($pdfdata, $filename);
        $this->current_parser =& $this->parsers[$filename];
        $this->setPDFVersion(max($this->getPDFVersion(), $this->current_parser->getPDFVersion()));
    ...
    protected function _getPdfParser($filename) {
        ...
        return new CustomTcpdiParser($data, $filename);
    }
    ...
    public function pdf_write_value(&$value)
    {
        if (!is_array($value)) {
            if (is_int($value) || is_float($value)) {
                $this->_straightOut($value . ' ');
            } elseif (is_bool($value)) {
                $this->_straightOut($value ? 'true ' : 'false ');
