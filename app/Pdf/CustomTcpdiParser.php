...
// -----------------------------------------------------------------------------
     ...
     */
    function readPages() {
        if (!isset($this->xref['trailer']) || !is_array($this->xref['trailer']) || !isset($this->xref['trailer'][1]) || !is_array($this->xref['trailer'][1]) || !isset($this->xref['trailer'][1]['/Root'])) {
            return;
        }
        $params = $this->getObjectVal($this->xref['trailer'][1]['/Root']);
        $objref = null;
        if (is_array($params) && isset($params[1]) && is_array($params[1]) && isset($params[1][1]) && is_array($params[1][1])) {
        ...
        if ($objref == null || !is_array($objref) || !isset($objref[0]) || $objref[0] !== PDF_TYPE_OBJREF) {
            // Offset not found.
            return;
        }

        $dict = $this->getObjectVal($objref);
        if (is_array($dict) && isset($dict[0]) && $dict[0] == PDF_TYPE_OBJECT && isset($dict[1]) && is_array($dict[1]) && isset($dict[1][0]) && $dict[1][0] == PDF_TYPE_DICTIONARY) {
            // Dict wrapped in an object
            $dict = $dict[1];
        }

        if (!is_array($dict) || !isset($dict[0]) || $dict[0] !== PDF_TYPE_DICTIONARY) {
            return;
        }

        $this->pages = array();
        if (isset($dict[1]) && is_array($dict[1]) && isset($dict[1]['/Kids'])) {
            $v = $dict[1]['/Kids'];
            if (is_array($v) && isset($v[0]) && $v[0] == PDF_TYPE_ARRAY && isset($v[1]) && is_array($v[1])) {
                foreach ($v[1] as $ref) {
    ...
    protected function getIndirectObject($obj_ref, $offset=0, $decoding=true) {
        ...
        do {
            ...
            if (($i > 0) AND (isset($objdata[($i - 1)][0])) AND ($objdata[($i - 1)][0] == PDF_TYPE_DICTIONARY) AND is_array($objdata[($i - 1)][1]) AND array_key_exists('/Length', $objdata[($i - 1)][1])) {
                ...
                if (is_array($lengthobj) && isset($lengthobj[0]) && $lengthobj[0] === PDF_TYPE_OBJREF) {
                    $lengthobj = $this->getObjectVal($lengthobj);
                    if (is_array($lengthobj) && isset($lengthobj[0]) && $lengthobj[0] === PDF_TYPE_OBJECT && isset($lengthobj[1])) {
                        $lengthobj = $lengthobj[1];
                    }
                }
                $streamlength = is_array($lengthobj) && isset($lengthobj[1]) ? $lengthobj[1] : $lengthobj;
    ...
    public function getObjectVal($obj) {
        ...
        if ($obj[0] == PDF_TYPE_OBJREF) {
            ...
            if (isset($this->objects[$key[0]][$key[1]])) {
                ...
                $res = $this->getRawObject($streaminfo[1], $this->objstreams[$objs[0]][$objs[1]]);
                $this->objects[$key[0]][$key[1]] = array($res[0]);
                $object = $this->objects[$key[0]][$key[1]];
            }
            if (is_array($object)) {
                $ret[1] = $object[0];
                if (isset($object[1]) && is_array($object[1]) && isset($object[1][0]) && $object[1][0] == PDF_TYPE_STREAM) {
    ...
    public function getPageBox($page, $box_index, $k) {
        ...
        $box = null;
        if (is_array($page) && isset($page[1]) && is_array($page[1]) && isset($page[1][1]) && is_array($page[1][1]) && isset($page[1][1][$box_index]))
            $box =& $page[1][1][$box_index];

        if (!is_null($box) && is_array($box) && $box[0] == PDF_TYPE_OBJREF) {
            $tmp_box = $this->getObjectVal($box);
            if (is_array($tmp_box) && isset($tmp_box[1])) {
                $box = $tmp_box[1];
            }
        }

        if (!is_null($box) && is_array($box) && $box[0] == PDF_TYPE_ARRAY) {
            $b =& $box[1];
        ...
        } elseif (!is_array($page) || !isset($page[1]) || !is_array($page[1]) || !isset($page[1][1]) || !is_array($page[1][1]) || !isset ($page[1][1]['/Parent'])) {
            return false;
        } else {
    ...
    private function _getPageRotation($obj) { // $obj = /Page
        $obj = $this->getObjectVal($obj);
        if (is_array($obj) && isset($obj[1]) && is_array($obj[1]) && isset($obj[1][1]) && is_array($obj[1][1]) && isset($obj[1][1]['/Rotate'])) {
            $res = $this->getObjectVal($obj[1][1]['/Rotate']);
            if (is_array($res) && $res[0] == PDF_TYPE_OBJECT)
                return $res[1];
            return $res;
        } else {
            if (!is_array($obj) || !isset($obj[1]) || !is_array($obj[1]) || !isset($obj[1][1]) || !is_array($obj[1][1]) || !isset($obj[1][1]['/Parent'])) {
                return false;
            } else {
